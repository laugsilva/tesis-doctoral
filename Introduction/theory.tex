...
Jet mass is the most basic observable for distinguishing a boosted object from an ordinary quark- or gluon-initiated jet.
...
(We can) use $N$-subjettiness to effectively ``count'' the number of subjets in a jet. Compare to previous jet substructure techniques, $N$-subjettiness  has the advantage of finding jets that contain two or more lobes of energy.
Consider for instance the decay of a boosted $W$ boson. Since the $W$ decays to two quarks, a single jet containing a boosted $W$ boson should be composed of two distinct hard subjets with a combined invariant mass around 80~GeV. A boosted QCD jet with an invariant mass of 80~GeV usually originates from a single hard parton and acquires mass through large angle soft splittings. Therefore the difference in expected energy flow between these two types of jets can be exploited by ``counting'' the number of hard lobes of energy within a jet.

Jets with $\tau_N\approx 0$ have all their radiation aligned with the candidate subjet directions and therefore have $N$ (or fewer) subjets. Jets with $\tau_N\gg 0$ have a large fraction of their energy distributed away from the candidate subjet direction and therefore have at least $N+1$ subjets.
A key step for defining $N$-subjettiness is to appropiately chosse the candidate subjet directions.
In its version such candidates where determined by using the exclusive $k_t$ algorithm, forcing it to return exactly $N$ jets.

Once the candidate subjets are identified, $N$-subjettiness is a proper inclusive jet shape. Since eq.~\ref{eqn:nsubjet} is linear in each of the constituent particle momenta, it is an infrared- and colliner-safe observable.
