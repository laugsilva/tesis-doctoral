...
The study of a quantity related to the distributions and multiplicity of particles in the event phase space lead to the first evidence of jet structure, as pointed out in ref.~\cite{PhysRevLett.35.1609}. In general, all final hadronic states in $pp/p\bar{p}/e^+e^-$ collisions can be explored in terms of the structure and shape of the energy flow by means of so called ``event shape'' variables. This family of variables attempt to extract information about the global geometry of event, usually distinguishing between di-jet events and multijet final states. Such variables have been successfully utilized in many SM measurements and BSM searches, see for example~\cite{Abbiendi:2007aa}\cite{Aad:2012np}. 
...
A method for selecting these jets would allow for the study of their properties.
  This interest lead to the development of a wide range of jet substructure techniques in the recent years.  Jet substructure methods probe the internal structure of jets from a detailed study of its constituents (see chapter\ref{sec:reconstruction}). These techniques have been first thought for distinguishing boosted hadronic objects from the background of jets initiated by light quarks and gluon, see for example~\cite{ATLASBoostedHbb},
...
The study of the distribution of particles within a jet has proven to be useful for distinguishing jets initiated by different flavors of quark or by a gluon, see for instance~\cite{PhysRevLett.107.172001}. The shape of a jet no only depends on the type of parton (quark or gluon) but is also sensitive to non-perturbative fragmentation effects and underlying event contributions~\cite{ATLASJetShapes}.
...
%From~\cite{PhysRevLett.107.172001}%Quark and Gluon Tagging at the LHC
We find the best single observable in the continuous category is the linear radial moment—a measure of the ‘‘width’’ or ‘‘girth’’ of the jet—constructed by adding up the pT deposits within the jet, weighted by the distance from the jet axis. It is defined as:
FORMULA
where ri 1⁄4 Áy2 þ Á2 and where the true boost-ii invariant rapidity y should be used for the (massive) jet axis instead of the geometric pseudorapidity . Under the assumption of central jets with massless constituents at small angles, this linear moment is identical to jet broadening, defined as the sum of momenta transverse to the jet axis normalized by the sum of momenta. While jet broadening is natural at an eþ eÀ collider, the linear radial moment is more natural and works a bit better at the LHC.


%From http://www.springerlink.com/content/54748j617818718q/?MUD=MP
A radial moment sums these distances (or a function of these distances), weighted by a quantity like pT , then normalized t o the total pT of the jet. For example, the linear radial moment, girth, is defined as [12]
...
Jet Angularities are also radial moments, but their “radial distances” are rescaled intothe angular coordinates appropriate for e+ e− event shapes. They are defined by [24]

This is a boost-invariant versionof the jet broadening, to which it reduces in the limit of massless constituents at small angles to the jet axis.


%Jet shapes and jet algorithms in SCET, by Ellis, Vermillon, et all
%The angularity τa of a jet, defined by (cf. [12, 17]), τa ≡ ... pi e−ηi (1−a) , where a is a parameter taking values −∞ < a < 2 (for IR safety, although factorizability will require a < 1), the sum is over all particles in the jet, EJ is the jet energy, pT is the transverse momentum relative to the jet direction, and η = − ln tan(θ/2) is the (pseudo)rapidity measured from the jet direction A small jet mass or angularity forces harder particles to be closer to the jet axis.

%Measurement of QCD jet broadening~\cite{PhysRevD.44.601}.In this paper, we discuss the use of an event-shape parameter QT introduced by Ellis and Webber to describe hadronic final states in proton-antiproton collisions [18]. QT is defined as the scalar sum of the momenta perpendicular to the transverse thrust axis, where the transverse thrust axis represents the direction of maximum energy flow in the plane transverse to the beam. It has the property of being both finite and calculable for all multiparton final-state configurations, the case where particularly singularities occur in perturbation theory associated with the collinear emission of gluons. The behavior of QT with increasing energy is termed jet broadening because of its sensitivity to the increasing multiparton nature of a jet as the energy of the jet increases.
% $Q_T / E_T$ ~ similar behavior with $E_T$ as Jet Width.


\subsubsection{Eccentricity?}

In constructing a jet moment, there are a number of ways to weight the momentum, such by energy or $\pt$, and to define the center of the jet. We have defined the jet width as a linear radial moment. Another effective combination is a $\pt$-weighted vector called pull~\cite{PhysRevLett.105.022001}. But is also natural to look at higher moments, such as those contained in the covariance tensor
...
The jet mass can be used to discriminate at a high-level between jets initiated by a massive object and those that are the result of succesive branchings of the parton shower. Massive jets formed by the latter are expected to be dominated by wide-angle emissions. %http://prd.aps.org/abstract/PRD/v79/i7/e074012
%...
There is an increase probability to see high mass jets initiated from a gluon as opposed to a quark.  And, large radii jets at high $\pt$ are expected to exhibit higher mass scales.
...
Radiation variable, which are dependent on the radiation pattern of the event~\cite{MultivariateDiscrim}.

By using succesive recombination algorithms such as those described in previus sectin, it is straightforward to define a ``subjet algorithm'' in which the structure of the constituents of a given jet are parsed using either the original algorithm or a new one at a fixed distance parameter. This concept was initially suggested in different works%  http://www.sciencedirect.com/science/article/pii/055032139290296N
%http://www.sciencedirect.com/science/article/pii/037026939291467N
in order to quantify the jet multiplicity at various kinematic scales. 
Subsequent to the development of the $k_t$ algorithm, 
%http://prd.aps.org/abstract/PRD/v48/i7/p3160_1
%http://www.sciencedirect.com/science/article/pii/037026939291467N
a similar approach was also used at hadron colliders.
%http://www.sciencedirect.com/science/article/pii/S0920563299007641
%http://iopscience.iop.org/1126-6708/1999/09/009/

In addition to fixed distance parameter subjets, it is also possible to ``unfold'' the jet algorithm, or to undo the last recombination step in a succesive recombination algorithm so as to identify decays products of a (massive)object. This approach is used in seveal jet grooming procedures, see for instance~\cite{pruning}.
...
$N$-subjettiness~\cite{nsubjettiness}. is jet shape that describes the energy flow of a jet with respect to candidate subjet axes.  

%From david's
This observable seeks to measure the extent to which a given jet is likely to be composed of $N$ subjets by first identifying a set of subjets and  then comparing the energy flow in the jet to the direction of these subjets.

\begin{equation} 
\tau^{(\beta)}_N = \frac{1}{\sum_k {\pt}_k\,(R_0)^{\beta}} \sum_k {\pt}_k (\min \{ \Delta R_{S_1,k},\,\Delta R_{S_2,k},...,\,\Delta R_{S_N,k} \})^{\beta}
\label{eqn:nsubjet}
\end{equation} 
...
An exponential weight $\beta$ may optinally be applied to the angular distance computed between the subjets and the jet constituents.


%From boost2011 summary
This jet shape was designed to identify boosted $N$-prong hadronic decays. 
$N$-subjettiness quantifies the degree to which a radiation is aligned along specified subjet axes, such that small values of $\tau_N$ correspond to N or fewer subjets, while larger values of $\tau_N$  indicate more than $N$ subjets.
This event shape was adapted from the event shape $N$-jettiness~\cite{njetti}.
Given candidate subjets directions determined by an external algorithm such as the exclusive $k_t$ procedure~\cite{exclusivekt}, the variables is defined as (EQUATION).
...
In subsequent work~\cite{mininsubjettiness}, Thaler and van Tilburg showed that the initial step of choosing candidate subjet axes is in fact unnecessary. In particular, the quantity in equation\label{eqn:nsubjet} can be minimised over the candidate subjet directions, further improving boosted object discrimination.
