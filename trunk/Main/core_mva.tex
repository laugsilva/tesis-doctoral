...
Two different likelihood configurations were evaluated: the simple likelihood ratio estimator,  using an adaptive Gaussian KDE strategy for the estimation of the PDFs of the input variables; and the more sophisticated PDE-RS approach, adopting an adaptive mode for the volume search.  The multidimentional PDE-RS classifier offers no gain in discrimination with respect to the more simpler likelihood method, with the further disadvange of being more time consuming.  The likelihood method shows good performance, and, given the low correlation of the input variables accross $\pt$, constitutes an adequate method for single-merged discrimination with a fast training step.
...
The outputs of the explored MVA discriminants are different in terms of shape and range, although the latter could be rearranged with a suitable variable transformation.  In spite of these distinct features the performances of the different methods, expressed in rejection of merged $b$-jets as a function of single $b$-jet efficiency (see Section~\ref{sec:btaggingalgos}), agree within statistics, see Fig.~\ref{fig:diffmethodsPerfBins}.  
...
The distribution of the likelihood output for single and merged $b$-jets is shown in  Fig.~\ref{fig:outputinbins} for medium and high transverse momentum jets.
