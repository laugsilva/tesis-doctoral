...
%\subsubsection{Use of displaced tracks}
%PYTHIA STANDALONE
%http://slac.stanford.edu/~swiatlow/gbb_plots07_07/ntrk_gbb_b_J3_PT80.png
%http://slac.stanford.edu/~swiatlow/gbb_plots07_07/ntrkB_gbb_b_J3_PT80.png
No improvement was obtained either by circumscribing the selection to tracks with large impact parameter significance($> 2.5$). The potential sensitivity achieved by enriching the sample in tracks associated to the $b$-hadron is counterbalanced by the lower number of associated tracks. A comparison of track multiplicity distributions using all tracks and distributions built with displaced tracks is shown in Fig.~\ref{fig:displacedntrk}.

\begin{figure}[tp]
\centering
\includegraphics[width=0.49\textwidth]{FIGS/TEMPFigs/DisplacedTracks/Ntrk_allanddisplaced080.pdf}
\includegraphics[width=0.49\textwidth]{FIGS/TEMPFigs/DisplacedTracks/Ntrk_allanddisplaced200.pdf}
...
%PYTHIA STANDALONE
%b-jets resemble gluon jets more closely: known result from LEP (REFERENCIA)
%this variable was used to separate gluons from quarks...
%80-120GeV http://slac.stanford.edu/~swiatlow/gbb_plots/trkWidth_J3_PT80.


Figure~\ref{fig:trkwidthsinglemerged} shows the distributions for the track-jet width for which the sum in equation~\ref{eqn:trackjetwidth} runs over the $N$ tracks associated to the jet, using the same criteria as for the jet track multiplicity. %There is a small number of jets at low pt that have only one track, because the selection criteria are not identical to those applied at the moment of b-tagging. Shape variables are not calculated for these jets, which are directly classified as single in the tagger presented in section 8. 
