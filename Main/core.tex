...
%For the subjet reconstruction a track-jet is built with all the tracks inside the jet using the $k_t$ jet algorithm~\cite{kt1}. A large radius is used in order to ensure that all tracks are clustered. Once the new jet is built a step back in the $k_t$ sequence is taken to obtained the two subjets that contributed to the final one. 
